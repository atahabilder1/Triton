...
class TritonSystem:
    ...
    def _load_checkpoints(self):
        ...
        try:
            # Load best checkpoint files (prioritize *_best.pt over epoch files)
            semantic_best = checkpoint_path / "semantic_encoder_best.pt"
            fusion_best = checkpoint_path / "fusion_module_best.pt"
            static_best = checkpoint_path / "static_encoder_best.pt"
            dynamic_best = checkpoint_path / "dynamic_encoder_best.pt"

            # Fallback to epoch files if best not available
            semantic_ckpts = [semantic_best] if semantic_best.exists() else sorted(glob.glob(str(checkpoint_path / "semantic_encoder_epoch*.pt")))
            fusion_ckpts = [fusion_best] if fusion_best.exists() else sorted(glob.glob(str(checkpoint_path / "fusion_module_epoch*.pt")))
            static_ckpts = [static_best] if static_best.exists() else sorted(glob.glob(str(checkpoint_path / "static_encoder_epoch*.pt")))
            dynamic_ckpts = [dynamic_best] if dynamic_best.exists() else sorted(glob.glob(str(checkpoint_path / "dynamic_encoder_epoch*.pt")))

            # Load semantic encoder (best checkpoint)
            if semantic_ckpts:
                semantic_ckpt = semantic_ckpts[-1]  # Use the best model or latest epoch
